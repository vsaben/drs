"""
    ...
    - Determine: yolo anchors (given target image size), maximum detections, class weighting
    - Number: images, detections (per dataset and image)
    - Median: detections per image, dimension anchors
    - Distribution: weather, class, rain, snow, occlusion, truncation
    
    Functions:
    - PART A: Read in, extract and process data
    - PART B: Calculate kmean anchors 
    - PART C: Write exploratory results (incl. anchors) to file and update configuration file
                if specified
    - PART D: Amalgamate functionality for train.py and cmd line use
"""
...
from methods._data_reformat import get_bb2d, VEHICLE_CLASSES, WEATHER_CLASSES
...
def process_data(data_path, image_size, nclusters):
    ...
    ndetect, median_detect = get_n_median(arr_n, 0)
    ndamaged, median_damaged = get_n_median(arr_n, 1)
    ...
    res_dict = {"centroids_lst": centroids_lst, 
                "nimage": nimage, 
                "ndetect": ndetect, 
                "median_detect": median_detect, 
                "ndamaged": ndamaged, 
                "median_damaged": median_damaged, 
                "max_boxes": max_boxes,
                "tru_freq": tru_freq, 
                "cls_freq": cls_freq, 
                "wthr_freq": wthr_freq, 
...
def get_fivesum(ds_lst, i):
    arr = np.concatenate(ds_lst[i])
    return np.percentile(arr, [0, 25, 50, 75, 100])

def get_n_median(arr_n, col_i):
    arr = arr_n[:, col_i]
    n = int(np.sum(arr))
...
def write_explore(output_dir, res_dict, image_size, cfg = None):
    ...
    f.write("Median Detections Per Image (damaged): {:d} ({:d})\n".format(r['median_detect'], r['median_damaged']))
    f.write("Class Weighting (damaged): {:.4f}\n".format(r['ndamaged']/r['ndetect']))
    f.write("Max Boxes: {:d}\n".format(r['max_boxes']))    
    ...
    if cfg is not None:
        ...
        cfg.MAX_GT_INSTANCES = 2 * r['max_boxes']

        cfg.YOLO.ANCHORS_6 = tf.constant(std_anchor_dict[6], tf.float32) # Slightly more accurate
        cfg.YOLO.ANCHORS_9 = tf.constant(std_anchor_dict[9], tf.float32)
...
def explore(data_path, output_dir, image_size, nclusters = 14, cfg = None):

    res_dict = process_data(data_path, image_size, nclusters)
    write_explore(output_dir, res_dict, image_size, cfg = cfg)
